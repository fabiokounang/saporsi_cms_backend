...
async function getAbout() {
  const pool = getPool();
  const [rows] = await pool.query(`SELECT * FROM site_about WHERE id = ?`, [1]);
  return rows[0] || null;
}
...
async function updateCards(items) {
  ...
  for (const it of items) {
    await pool.query(`UPDATE site_about_cards SET card_type=?, title_id=?, title_en=?, description_id=?, description_en=?, theme=?, icon_key=?, sort_order=?, is_active=? WHERE id = ?`,
      [
        it.card_type || "",
        it.title_id || "",
        it.title_en || "",
...
async function deleteCard(id) {
  const pool = getPool();
  await pool.query(`DELETE FROM site_about_cards WHERE id=?`, [Number(id)]);
}
...
async function deletePoint(id) {
  const pool = getPool();
  await pool.query(`DELETE FROM site_about_points WHERE id=?`, [Number(id)]);
}
